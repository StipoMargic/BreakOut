...
void Paddle::Update(uint32_t dt, Ball& ball)
{
    ...
    if (mDirection != PaddleDirection::NONE)
    {
        ...
        else if (IsGreaterThanOrEqual(aaRect.GetBottomRight().GetX(),
                                      mBoundary.GetBottomRight().GetX()))
        {
            MoveTo(Vec2D(mBoundary.GetBottomRight().GetX() - aaRect.GetWidth(),
                         aaRect.GetTopLeft().GetY()));
        }
    }
}
