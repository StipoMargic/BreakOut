...
void Ball::MakeFlushWithEdge(const BoundaryEdge& edge, Vec2D& pointOnEdge, bool limitToEdge)
{
    ...
    {
        mBBox.MoveTo(
            Vec2D(mBBox.GetTopLeft().GetX(), edge.edge.GetPoint0().GetY() + edge.normal.GetY()));
    }
    else if (edge.normal == UP_DIR)
    {
        mBBox.MoveTo(
            Vec2D(mBBox.GetTopLeft().GetX(), edge.edge.GetPoint0().GetY() - mBBox.GetHeight()));
    }
    else if (edge.normal == RIGHT_DIR)
    {
        mBBox.MoveTo(
            Vec2D(edge.edge.GetPoint0().GetX() + edge.normal.GetX(), mBBox.GetTopLeft().GetY()));
    }
    else if (edge.normal == LEFT_DIR)
    {
        mBBox.MoveTo(
            Vec2D(edge.edge.GetPoint0().GetX() - mBBox.GetWidth(), mBBox.GetTopLeft().GetY()));
    }
