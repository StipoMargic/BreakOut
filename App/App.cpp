...
bool App::Init(uint32_t width, uint32_t height, uint32_t magnification)
{
...

    {
        ...
        std::unique_ptr<GameScene> breakoutScene =
            std::make_unique<GameScene>(std::move(breakoutGame));
...
void App::Run()
{
    ...
    if (mnoPtrWindow)
    {
        ...
        mInputController.Init([&isOpen](uint32_t dt, InputState state) { isOpen = false; });
